...
class AmiTask(object):
    """Base class for Pulp CLI tasks

    Instances for AmiTask subclass may be obtained to request an AMI
    tasks like push, delete etc.

    This class provides a CLI parser for the AMI tasks. Parser is
    configured with minimal options which can be extended by subclass.
    """

    def __init__(self):
        super(AmiTask, self).__init__()
    ...
    def add_args(self):
        ...
        from_super = getattr(super(AmiTask, self), "add_args", lambda: None)
        from_super()
